...
/**
 * Admin class that handle network post data.
 */
final class Papi_Admin_Network_Handler extends Papi_Core_Data_Handler {

	/**
	 * Save properties with a post id of zero.
	 */
	public function save_properties() {
		if ( $_SERVER ['REQUEST_METHOD'] !== 'POST' || papi_get_meta_type() !== 'network' ) {
			return;
		}
		...
		foreach ( $data as $key => $value ) {
			papi_data_update( 0, $key, $value, 'network' );
		}
		 ...
		 */
		do_action( 'papi/save_properties', 0, 'network' );
	}
...
if ( papi_is_admin() ) {
	new Papi_Admin_Network_Handler;
}
